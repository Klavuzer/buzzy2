...
<div class="content">
    <div class="reaction-emojis" style="float:none;margin:30px auto;text-align:center">
        ...
        <a {{ $reaction==$react->reaction_type ? 'class=selected' : '' }}
            href="{{ action('PagesController@showReaction', ['reaction' =>$react->reaction_type] ) }}"
            title="{{ $react->name }}" style="padding:7px;display: inline-block"><img alt="{{ $react->name }}"
                src="{{ $react->icon }}" width="55">
