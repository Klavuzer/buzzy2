...
<div class="entry" data-type="poll" data-co="{{ $uniquid }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}"
    ...
    <div class="inpunting mediaupload" @if(isset($entry->image)) style="display: none;" @endif>
        <div class="item-media-placeholder" style="padding-top: 70px;padding-bottom: 70px">
            ...
            <div class="image-upload-actions">
                <a class="button button-white getimageurl" data-action="add" data-target="entry">{{
                    trans('updates.getfromurl') }} <i class="fa fa-download"></i></a>
            </div>
        </div>
    </div>
    ...
    <div class="moredetail questionmore">
        ...
        <div id="answer{{ $uniquid }}"
            class="answers clearfix @if(isset($entry->video)) @if($entry->video == '1')thdefault @else {{  $entry->video == "
            2" ? 'thlarge' : 'thlist' }}@endif @else thlist @endif"
        ...
        <a class="button button-white button-full submit-button entry_fetch answerbutton clearfix" data-method="Get"
            ...
            href="{{ action('FormController@addnewform') }}?addnew=pollanswer"><i class="fa fa-plus-circle"></i>{{
            trans('addpost.add', ['type' => trans('buzzyquiz.answer')]) }}</a>
    </div>
</div>
