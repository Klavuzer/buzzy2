        ...
        @if(get_buzzy_config('UserEditEmail')=='yes' or Auth::user()->usertype=='Admin')
            <div class="form-group">
               ...
               @if(get_buzzy_config('UserVerifyEmail')=='yes')
                ...
                <span class="verified"><i class="material-icons" style="margin-right:5px;">verified</i> {{__('This email is verified')}}</span>
                @else
                 <span class="not_verified"><i class="material-icons" style="margin-right:5px;">remove_circle_outline</i> {{__('Not verified')}}</span> <a href="{{route('verification.notice')}}">{{__('Click here to verify your email address.')}}</a>
                @endif
                </div>
                @endif
            </div>
        ...
        @if(get_buzzy_config('UserAddSocial')=='yes' || Auth::user()->usertype=='Admin')
            ...
            @foreach ($social_links as $provider => $social)
                <div class="form-group">
                    ...
                    {!! Form::text('social_profiles['.$provider.']', $userinfo->social_profiles[$provider] ?? null, ['class' => 'cd-input','id' => $provider]) !!}
                </div>
            @endforeach
        @endif
