    ...
    <div class="user-info {{ $post->user->genre }} answerer">
        ...
        <div class="info">
            <a itemprop="name" class="content-info__author"
                href="{{ action('UsersController@index', ['userslug' => $post->user->username_slug ]) }}"
            ...
            @endif
            <div class="detail">
                {!! trans('index.postedon', ['time' => '<time
                    class="content-info__date">'.$post->published_at->diffForHumans() .'</time>' ]) !!}
                @if($post->updated_at->getTimestamp() > $post->published_at->getTimestamp())
                <em class="content-info__line">—</em> {!! trans('index.updatedon', ['time' => '<time
                    class="content-info__date">'.$post->updated_at->diffForHumans() .'</time>' ]) !!}
                @endif
            </div>
        </div>
    </div>
