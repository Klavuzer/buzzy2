...
class PostEditorController extends Controller
{
    ...
    private function savePost($inputs, $post, $update = false)
    {
        try {
            ...
            $post->language = isset($inputs['language']) ? $inputs['language'] : get_buzzy_config('sitedefaultlanguage', 'en');
            ...
            $tagIds = !empty($inputs['tags']) ? collect(explode(',', $inputs['tags']))->map(function ($tagName) use ($post) {
                return Tag::firstOrCreate([
                    'name' => $tagName,
                    'slug' => str_slug($tagName, '-', $post->language),
                    'type' => 'post_tag'
                ]);
            })->pluck('id') : [];
    ...
    private function createEntries($entries, $post_id)
    {
        foreach ((array) $entries as $entry_key => $_entry) {
            ...
            if ($entry_type == "quizquestion" || $entry_type == "poll") {
                ...
                $entry->video = isset($_entry['video']) ? str_replace(array('##', '%#'), array('=', '<'), $_entry['video']) : null;
            }
    ...
    private function filterEntryContent($content, $entry_key, $post_id)
    {
        ...
        if (is_array($matches) && !empty($matches)) {
            foreach ($matches[1] as $img) {
                if (Str::startsWith($img, '/upload/tmp/') || Str::startsWith($img, 'upload/tmp/')) {
                    $newImage = makepreview($this->moveEntryImage($img, $post_id, 'text', $entry_key, null), null, 'entries');
