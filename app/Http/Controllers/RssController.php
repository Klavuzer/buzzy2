...
class RssController extends Controller
{
    ...
    {
        $posts = Post::with('user')->where('type', '!=', 'quiz')->byPublished()->byLanguage(request()->get('lang'))->byApproved()->limit(150)->get();
    ...
    public function json($type)
    {
        ...
        if ($posts) {
            foreach ($posts as $key => $post) {
                $result[] = array('slug' => generate_post_url($post), 'title' => $post->title, 'thumb' =>  makepreview($post->thumb, 's', 'posts'));
            }
        }
