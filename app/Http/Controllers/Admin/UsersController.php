    ...
    public function getTableData(Request $request)
    {
        ...
        return Datatables::of($user)
            ...
            ->editColumn('username', function ($user) {
                $url = url('/profile/' . $user->username_slug);
                $out = '<a href="' . $url . '"  target="_blank" > ' . $user->username . '  </a><div class=clear></div>';
                ...
                foreach ($social_links as $provider => $item) {
                    $out .= '<a href="' . $social_profiles[$provider] . '" target="_blank" style="margin-right:5px;margin-top:5px"><img width="26px" src="' . $item['icon'] . '" /></a>';
                }
            ...
            ->editColumn('email', function ($user) {
                ...
                if ($user->email_verified_at) {
                    $output .= '<i data-toggle="tooltip" data-original-title="' . __('Verified') . '" class="fa fa-check-circle" style="margin-left:5px;color:green"></i>';
                } else {
                    $output .= '<i data-toggle="tooltip" data-original-title="' . __('Not verified') . '" class="fa fa-times-circle" style="margin-left:5px;color:#ccc"></i>';
                }
