...
class CommentController extends MainAdminController
{
    ...
    public function showPostComments(Request $request, $id)
    {
        ...
        return view('_admin.pages.comments')->with(['title' => $comment->title, 'desc' => $comment->title, 'type' => 'category', 'type' => 'comment', 'data_url' => $data_url]);
    }
        ...
        return Datatables::of($comment)
            ...
            ->editColumn('comment', function ($comment) {
                ...
                if (Str::length($comment->comment) > 100) {
                    $output = '<div class="comment-content" onClick="expandComment(this)">';
            ...
            ->editColumn('post', function ($comment) {
                ...
                return '<a href="' . generate_comment_url($comment) . '" target=_blank style="font-size:16px;font-weight: 600">
                        ' . $comment->post->title . '
                        </a>
            ...
            ->editColumn('user', function ($comment) {
                ...
                return '<a href="' . $comment->userdata->link . '" target="_blank"><img src="' . $comment->userdata->icon . '" width="32" style="margin-right:6px">' . $comment->userdata->username . '</a>';
            })
            ->addColumn('approve', function ($comment) {
                if ($comment->deleted_at !== null) {
                    $result = '<div class="label label-danger">' . trans("admin.OnTrash") . '</div>';
                } elseif (!$comment->approve) {
                    $result = '<div class="label label-info" style="background-color: #9c6a11 !important;">' . trans("admin.AwaitingApproval") . '</div>';
                } elseif ($comment->approve) {
                    $result = '<div class="label label-info">' . trans("admin.Active") . '</div>';
                }

                if ($comment->reports_count > 0) {
                    $result .= '<div class="label label-warning" style="margin-left: 4px">' . __('Reported') . '</div>';
                }

                if ($comment->userdata->type == 'guest') {
                    $result .= '<div class="label label-success" style="margin-left: 4px">' . __('Guest') . '</div>';
                }

                if ($comment->parent_id) {
                    $result .= '<div class="label label-default" style="margin-left: 4px">' . __('Reply') . '</div>';
                }
            ...
            ->addColumn('action', function ($comment) {
                ...
                if ($comment->reports_count > 0) {
                    $result = $result . '<li><a href="javascript:void(0);" onClick="getCommentReports(this)" data-url="' . action("Admin\CommentController@getCommentReportsModal", ['id' => $comment->id]) . '"><i class="fa fa-flag"></i>  ' . __("View Reports") . '</a></li>';
