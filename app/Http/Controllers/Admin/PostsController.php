    ...
    public function getTableData(Request $request)
    {
        ...
        return Datatables::of($post)
            ...
            ->editColumn('title', function ($post) {
                ...
                $comments = get_buzzy_config('p_buzzycomment') == 'on' ? '<a href="' . action('Admin\CommentController@index', ['type' => 'post', 'post_id' => $post->id]) . '">' . __(':count Comments', ['count' => $post->comments_count]) . '</a>' : '';

                return '<a href="' . generate_post_url($post) . '" target=_blank style="font-size:16px;font-weight: 600">
                        ' . $post->title . '
                        </a>
                        <p style="margin-top:5px;color:#ccc">' . Str::limit($post->body, 45) . '</p>
